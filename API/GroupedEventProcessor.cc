...
void GroupedEventProcessor::Process(const std::shared_ptr<const JEvent>& event) {
	...
	// If no JANA plugin is attached that can provide TriggerDecision
	// objects, then the GetAll() call will throw an exception. We don't
	// want that so check first if any such factories exist.
	auto factories = event->GetFactoryAll<TriggerDecision>(false);
	if( ! factories.empty() ){
